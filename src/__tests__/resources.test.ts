...
import { Authorizations } from '../resources/authorizations.js';
import { HitlConfigs } from '../resources/hitl.js';
import { Confirmations } from '../resources/confirmations.js';
...
describe('VerificationRequests resource', () => {
  ...
  it('getProofs sends GET to /:id/proofs', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ proofs: [{ type: 'phone', proof_token: 'jwt_proof' }] });
    const vr = new VerificationRequests(http);

    const result = await vr.getProofs('vr_123');

    expect(http.get).toHaveBeenCalledWith('/api/v1/verification-requests/vr_123/proofs');
    expect(result.proofs).toHaveLength(1);
    expect(result.proofs[0].proof_token).toBe('jwt_proof');
  });
});
...
// AUTHORIZATIONS
// ============================================================
describe('Authorizations resource', () => {
  it('create sends POST to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ id: 'auth_123', status: 'pending' });
    const a = new Authorizations(http);
    ...
    expect(http.post).toHaveBeenCalledWith('/api/v1/authorizations', {
      phone: '+15551234567',
      channel: 'whatsapp',
      business_name: 'Acme Corp',
    });
  ...
  it('create includes optional params', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ id: 'auth_123' });
    const a = new Authorizations(http);
    ...
    await a.create({
      ...
      business_name: 'Acme',
      scope: 'confirmations',
      business_profile_id: '507f1f77bcf86cd799439011',
      expires_in: 86400,
    });
  ...
  it('retrieve sends GET to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ id: 'auth_123' });
    const a = new Authorizations(http);

    await a.retrieve('auth_123');

    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations/auth_123');
  });
  ...
  it('retrieve encodes special characters in ID', async () => {
    ...
    await a.retrieve('auth/special&id');

    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations/auth%2Fspecial%26id');
  });
  ...
  it('list sends GET with query params', async () => {
    ...
    await a.list({ status: 'active', channel: 'whatsapp', limit: 10 });

    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations', { status: 'active', channel: 'whatsapp', limit: 10 });
  });

  it('list sends GET without params when none provided', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ data: [], pagination: {} });
    const a = new Authorizations(http);
    ...
    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations', undefined);
  });

  it('revoke sends DELETE with reason', async () => {
    const http = mockHttp();
    vi.mocked(http.del).mockResolvedValue({ id: 'auth_123', status: 'revoked' });
    const a = new Authorizations(http);

    await a.revoke('auth_123', { reason: 'No longer needed' });

    expect(http.del).toHaveBeenCalledWith('/api/v1/authorizations/auth_123', { reason: 'No longer needed' });
  });

  it('revoke sends DELETE without reason', async () => {
    const http = mockHttp();
    vi.mocked(http.del).mockResolvedValue({ id: 'auth_123', status: 'revoked' });
    const a = new Authorizations(http);

    await a.revoke('auth_123');

    expect(http.del).toHaveBeenCalledWith('/api/v1/authorizations/auth_123', undefined);
  });

  it('export sends GET with query params', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ data: [], total: 0 });
    const a = new Authorizations(http);

    await a.export({ format: 'json', status: 'active' });

    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations/export', { format: 'json', status: 'active' });
  });

  it('export sends GET without params', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ data: [], total: 0 });
    const a = new Authorizations(http);
    ...
    expect(http.get).toHaveBeenCalledWith('/api/v1/authorizations/export', undefined);
  });
});
...
describe('Confirmations resource', () => {
  it('create sends POST to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ id: 'conf_123', status: 'pending' });
    const c = new Confirmations(http);

    await c.create({ action: 'deploy:prod', message: 'Deploy v2.3?', hitl_id: 'hitl_123' });

    expect(http.post).toHaveBeenCalledWith('/api/v1/confirmations', {
      action: 'deploy:prod',
      message: 'Deploy v2.3?',
      hitl_id: 'hitl_123',
    });
  ...
  it('create includes optional params', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ id: 'conf_123' });
    const c = new Confirmations(http);
  ...
  it('retrieve sends GET to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ id: 'conf_123' });
    const c = new Confirmations(http);

    await c.retrieve('conf_123');

    expect(http.get).toHaveBeenCalledWith('/api/v1/confirmations/conf_123');
  });

  it('retrieve encodes special characters in ID', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ id: 'conf/special&id' });
    const c = new Confirmations(http);

    await c.retrieve('conf/special&id');

    expect(http.get).toHaveBeenCalledWith('/api/v1/confirmations/conf%2Fspecial%26id');
  });
  ...
  it('list sends GET with query params', async () => {
    ...
    await c.list({ status: 'pending', hitl_id: 'hitl_1', limit: 10 });

    expect(http.get).toHaveBeenCalledWith('/api/v1/confirmations', { status: 'pending', hitl_id: 'hitl_1', limit: 10 });
  });
  ...
  it('list sends GET without params when none provided', async () => {
    ...
    expect(http.get).toHaveBeenCalledWith('/api/v1/confirmations', undefined);
  });
});
...
describe('HitlConfigs resource', () => {
  it('create sends POST to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ id: 'hitl_123', status: 'active' });
    const h = new HitlConfigs(http);

    await h.create({ name: 'Prod Approvals', channel: 'telegram' });

    expect(http.post).toHaveBeenCalledWith('/api/v1/hitl', { name: 'Prod Approvals', channel: 'telegram' });
  });

  it('retrieve sends GET to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ id: 'hitl_123' });
    const h = new HitlConfigs(http);

    await h.retrieve('hitl_123');

    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl/hitl_123');
  });
  ...
  it('retrieve encodes special characters in ID', async () => {
    ...
    await h.retrieve('hitl/special&id');

    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl/hitl%2Fspecial%26id');
  });
  ...
  it('list sends GET with query params', async () => {
    ...
    await h.list({ status: 'active', limit: 10 });

    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl', { status: 'active', limit: 10 });
  });
  ...
  it('list sends GET without params when none provided', async () => {
    ...
    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl', undefined);
  });

  it('update sends PATCH to correct endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.patch).mockResolvedValue({ id: 'hitl_123', name: 'Updated' });
    const h = new HitlConfigs(http);

    await h.update('hitl_123', { name: 'Updated', status: 'archived' });

    expect(http.patch).toHaveBeenCalledWith('/api/v1/hitl/hitl_123', { name: 'Updated', status: 'archived' });
  });
  ...
  it('delete sends DELETE to correct endpoint', async () => {
    ...
    await h.delete('hitl_123');

    expect(http.del).toHaveBeenCalledWith('/api/v1/hitl/hitl_123');
  });

  it('requestAuthorization sends POST to authorize endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ hitl_id: 'hitl_123', authorizations: [], message: 'Sent' });
    const h = new HitlConfigs(http);

    await h.requestAuthorization('hitl_123');

    expect(http.post).toHaveBeenCalledWith('/api/v1/hitl/hitl_123/authorize', {});
  });

  it('createChatIdDiscovery sends POST to chat-id-discovery endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.post).mockResolvedValue({ token: 'abc', deep_link: 'https://t.me/bot', qr_code: 'data:image', expires_in: 600 });
    const h = new HitlConfigs(http);

    await h.createChatIdDiscovery();

    expect(http.post).toHaveBeenCalledWith('/api/v1/hitl/chat-id-discovery', {});
  });

  it('pollChatIdDiscovery sends GET to chat-id-discovery/:token endpoint', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ status: 'pending' });
    const h = new HitlConfigs(http);

    await h.pollChatIdDiscovery('abc123');

    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl/chat-id-discovery/abc123');
  });

  it('pollChatIdDiscovery encodes special characters in token', async () => {
    const http = mockHttp();
    vi.mocked(http.get).mockResolvedValue({ status: 'completed', chat_id: '12345' });
    const h = new HitlConfigs(http);

    await h.pollChatIdDiscovery('tok/en&special');

    expect(http.get).toHaveBeenCalledWith('/api/v1/hitl/chat-id-discovery/tok%2Fen%26special');
  });
});
